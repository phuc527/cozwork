...
export const HandleVideoCall: FC<Props> = ({ ticket }) => {
    ...
    const DropdownItem: FC<any> = (props) => {
        ...
        return (
            <>
                <StyledMenuItem variant="texted" onClick={handleOnClick} disabled={props.loading} className={classNames({
    ...
    return (
        <>
            <StyledDropdown>
                ...
                >
                    <StyledMenu className="dropdownOutSide">
                        <DropdownItem onClick={() => handleCreateRoom()} loading={loading}>Start a video call</DropdownItem>
                    </StyledMenu>
                </CSSTransition>
            </StyledDropdown>
        </>
...
const ProfileInfo: FC<Props> = ({ ticket }) => {
    ...
    const errorMessage = useAppSelector((store) => store.ticket.daily.errorMessage);
    const errorMessagePhone = useAppSelector((store) => store.ticket.daily.errorMessagePhone);
                    ...
                    <StyledBtns>
                        ...
                        <ul ref={containerRef} style={{ marginLeft: '8px' }}>
                            <StyledNavBtn
                                ...
                                id="videoCall"
                            >
                                <StyledText onClick={() => { handleMeeting(); setOpen(!open) }}>
                                    <Video />
                                </StyledText>
